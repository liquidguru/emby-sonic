...
- Test against local music library on coordinator-host
- **Deliverable:** Working API accessible at `http://coordinator-host:8765`
...
| **B. Sidecar / Docker** | Python service runs separately; plugin points at its URL | Zero binaries in plugin (current coordinator-host setup); user provisions the service |
