...
- **M5.1 — Mini-player dissolve polish (2026-06-20, built + Pixel 8 Pro
  verified):** During a real music crossfade, the mini-player now cross-dissolves
  the full track identity block (artwork, title, and artist) instead of only the
  artwork. Controls and the Cast button stay outside the overlay so the bar
  remains stable during the transition. A fresh-session notification shade check
  still showed full media metadata and controls.
- **M5.2 — Home startup polish (2026-06-20, built + Pixel 8 Pro verified):**
  Home now clears its startup loading state as soon as library discovery
  succeeds and then fills each section independently, so one slow Emby/coordinator
  row no longer blanks the whole first screen. The 12-item Recently Added album
...
- **M5.5 — Coordinator Sonic Adventure dedupe (2026-06-20, deployed +
  live-verified):** `build_adventure` now dedupes the interpolated middle by
  normalized artist+title identity, not just Emby item id, while still pinning
  the requested start/end tracks. The coordinator over-walks the A->B embedding
  path, searches a wider nearest-neighbour pool at each step, and evenly samples
  the deduped candidates back to the requested middle length so the resulting
...
- **M5.9 — Library drill-down latency polish (2026-06-20, built + Pixel 8 Pro
  verified):** Music Library now lazy-loads only the visible tab on entry, then
  loads Albums/Genres when those tabs are selected. This prevents the expensive
  album-art fallback pass from competing with an immediate Artists -> Artist
  drill-down after app resume. Artist detail also renders its album list as soon
  as Emby returns the albums; missing album-art fallback hydration and sonic
  similar rails update afterward. User verification on the paused/background
  resume path confirmed Music -> Artists -> Pink opens quickly, while the
  offline prefetch buffer status continued to respond correctly through a
  Wi-Fi off/on test.
- **M5.10 — Now Playing Similar tab (2026-06-20, built + Pixel 8 Pro
  verified):** the Now Playing "Similar" tab now calls the coordinator
  `/sonic/tracks/{id}/similar` endpoint instead of showing the placeholder. It
  loads nearest-neighbour tracks for the current song, hydrates their artwork
  through Emby, supports per-row playback, "Play similar" from the top, and a
  manual refresh. User verification confirmed the tab works well enough for the
  current alpha; similarity quality/tuning remains something to revisit with
  beta-tester feedback.
- **M5.11 — Now Playing queue edit controls (2026-06-20, built + Pixel 8 Pro
  verified):** the Queue tab now supports a compact first-pass edit flow for
  upcoming tracks: move up, move down, and remove. The currently playing item and
  already-played items are protected; row tap still jumps playback to a future
  item. `PlaybackController` remaps per-item Emby `PlaySessionId`s and stream
  offsets when future items move/remove, updates the active Media3 queue, clears
  Guest DJ's append attempt signature, and reschedules offline prefetch from the
...
- **M5.15 — Plugin auto-scan fix (2026-06-20, plugin v0.4.0.0, built):** the
  ...
  **0.4.1.0 follow-up (2026-06-21):** deploying revealed the deeper reason
  ...
  `ILogger<T>` dependency. Deployed to coordinator-host via NAS relay (build on
...
- **M5.19 — Volume normalisation (2026-06-25, merged to master):** per-track loudness
  ...
  backend deployed to coordinator-host and the `/sonic/tracks/loudness` endpoint verified
...
- **M5.20 — Windowless analysis worker (2026-06-25, merged to master):** the
  ...
  pythonw has no stdout. Verified on dev-pc.
...
**M3.5 verification (dev-pc / Pixel_3a_API_36, 2026-06-09):**
- Built with `./gradlew :app:assembleDebug`.
- Installed `android/app/build/outputs/apk/debug/app-debug.apk` with `adb install -r`.
...
**M3.6 verification (dev-pc / Pixel_3a_API_36, 2026-06-09):**
- Built with `./gradlew :app:assembleDebug`.
- Installed `android/app/build/outputs/apk/debug/app-debug.apk` with `adb install -r`.
...
**M3.8 verification (dev-pc / Pixel_3a_API_36, 2026-06-09/10):**
- Built with `./gradlew :app:assembleDebug`.
- Installed `android/app/build/outputs/apk/debug/app-debug.apk` with `adb install -r`.
...
**M3.9 verification (dev-pc / Pixel_3a_API_36, 2026-06-10):**
- Built with `./gradlew :app:assembleDebug`.
- Installed `android/app/build/outputs/apk/debug/app-debug.apk` with `adb install -r`.
...
**M4.3/M4.4 current verification state (dev-pc, 2026-06-11):**
