...
.DESCRIPTION
  ...
  Handles the two things that trip up a hand-rolled setup:
    * panns_inference hardcodes ~/panns_data. Under SYSTEM the home dir is
      \systemprofile (no model there) -> the generated wrapper pins USERPROFILE
...
# --- write the wrapper the task will run -------------------------------------
$wrapper = Join-Path $RepoDir 'worker_run.generated.ps1'
...
$lines = @(
    ...
    "`$env:USERPROFILE = '$profFwd'",        # so panns_inference finds ~/panns_data
    "`$env:COORDINATOR_URL = '$CoordinatorUrl'",
    "Set-Location '$repoFwd'",
    "& '$pyFwd' worker.py *>> '$logPath'"
)
Set-Content -Path $wrapper -Value $lines -Encoding ASCII
Write-Host "Wrapper written -> $wrapper" -ForegroundColor Green

# --- register the scheduled task --------------------------------------------
$taskArg = "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$wrapper`""
...
if ($Mode -eq 'service') {
    ...
    schtasks /Create /TN $TaskName /TR "powershell.exe $taskArg" /SC ONIDLE /I $IdleMinutes /F | Out-Null
